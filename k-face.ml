...
assignSort (('a, 'b) simpleK * (('a, 'b) simpleK * (('a, 'b) simpleK * 'c))) list
        => ('b metaVar * 'a sort) list

  type ('a, 'b) simpleK = SimId of 'b metaVar * 'a sort |
    SimTerm of 'a label * ('a, 'b) simpleK list | SimLabel of 'a label |
    SimEmpty of 'a sort
...
let rec getSortInNeList l = match l with Single (NonTerminal s) -> [s] | Single (Terminal s) -> []
           | Con (NonTerminal s, xl) -> insertToSet s (getSortInNeList xl) | Con (Terminal s, xl) -> getSortInNeList xl;;

let rec getSort s = match s with Syntax (a,b,c) -> insertToSet a (getSortInNeList b) | Subsort (a,b) -> insertToSet a [b]
                 | Token (a,b,c) -> [a]
                 | ListSyntax (a,b,c,d) -> insertToSet a [b];;										

let rec getSorts l = match l with [] -> [] | x::xl -> insertsToSet (getSort x) (getSorts xl);;

let rec getTerminals l = match l with Single (NonTerminal s) -> [] | Single (Terminal s) -> [s]
           | Con (NonTerminal s, xl) -> (getTerminals xl) | Con (Terminal s, xl) -> insertToSet s (getTerminals xl);; 

let rec getSynTerminal s = match s with Syntax (a,b,c) -> 
     if List.exists (fun a -> a = OnlyLabel) c then [] else (getTerminals b) | Subsort (a,b) -> []
                 | Token (a,b,c) -> []
               | ListSyntax (a,b,c,d) ->
...
let rec getSynTerminals l = match l with [] -> [] | x::xl -> insertsToSet (getSynTerminal x) (getSynTerminals xl);;

let builtinSorts = [K.Bool; K; KItem; KLabel; KResult; KList; List; Seta; Map; Bag; Id; String; Int; Float];;
...
let builtinTerminals = ["(";")";".Bag";".Map";".Set";".List";".K";"ListItem";"SetItem";"[";"[";"|->";"<-";"~>"];;
...
let printNewTerminals l = let rec printNewTerminalsAux l num = 
 match l with [] -> ("",builtinTerminalMap)
          | x::xl -> if List.exists (fun (a,b) -> a = x) builtinTerminalMap then printNewTerminalsAux xl num
...
let rec kSyntaxToData x pi regMap labelMap num = match x with Syntax (a,b,c)
   ...
   | Subsort (a,b) -> (SubsortData (a,b,pi), regMap,labelMap, num)
